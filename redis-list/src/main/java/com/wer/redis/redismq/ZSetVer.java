...
public class ZSetVer {
    ...
    public void producer(){
        ...
        try {
            ...
            for (int i = 0; i < 5; i++) {
                String orderId = "0000"+i;
                double score = System.currentTimeMillis() + (i*1000);
                jedis.zadd(RS_ZS_MQ_NS+"orderId",score,orderId);
    ...
    public void consumerDelayMessage(){
        ...
        try {
            jedis = jedisPool.getResource();
            while (true) {
                Set<String> orderId = jedis.zrangeByScore(RS_ZS_MQ_NS + "orderId", 0, System.currentTimeMillis(), 0, 1);
                ...
                String s = orderId.iterator().next();
